function [H, pValue, Qstatistic, criticalValue] = lbqtest1(Series , Lags , alpha , DoF)
...
%
%   pValue - Vector of p-values of the test statistics.
%
...
%   $Revision: 1.1.8.3 $  $Date: 2008/09/29 16:32:57 $
...
if (rows ~= 1) && (columns ~= 1) 
    error('econ:lbqtest:NonVectorSeries' , ' Input ''Series'' must be a vector.');
end
...
if (nargin >= 2) && ~isempty(Lags)
   if numel(Lags) == length(Lags)    % Check for a vector.
      rowLags  =  size(Lags,1) == 1;
   else
      error('econ:lbqtest:NonVectorLags' , ' ''Lags'' must be a vector.');
   end
   Lags  =  Lags(:);
   if any(round(Lags) - Lags) || any(Lags <= 0)
      error('econ:lbqtest:NonPositiveIntegerLags' , ' All elements of ''Lags'' must be positive integers.')
   end
   if any(Lags > (n - 1))
      error('econ:lbqtest:LagsTooLarge' , ' All elements of ''Lags'' must not exceed ''Series'' length - 1.');
   end
else
...
if (nargin >= 3) && ~isempty(alpha)
   if numel(alpha) ~= length(alpha)    % Check for a vector.
      error('econ:lbqtest:NonVectorAlpha' , ' ''Alpha'' must be a vector.');
   end
   alpha  =  alpha(:);
   if any(alpha <= 0 | alpha >= 1)
      error('econ:lbqtest:InvalidAlpha' , ' All significance levels ''Alpha'' must be between 0 and 1.'); 
   end
   if length(alpha) == 1
      alpha  =  alpha(ones(length(Lags),1));    % Scalar expansion.
   end
   if length(alpha) ~= length(Lags)
      error('econ:lbqtest:AlphaLagsLengthMismatch' , ' Sizes of ''Alpha'' and ''Lags'' must be the same.');
   end
else
   alpha  =  0.05;
end
...
if (nargin >= 4) && ~isempty(DoF)
   if numel(DoF) ~= length(DoF)    % Check for a vector.
      error('econ:lbqtest:NonVectorDoF' , ' ''DoF'' must be a vector.');
   end
   DoF  =  DoF(:);
   if any(round(DoF) - DoF) || any(DoF <= 0)
      error('econ:lbqtest:NonPositiveIntegerDoF' , ' All elements of ''DoF'' must be positive integers.')
   end
   if length(DoF) == 1
      DoF  =  DoF(ones(length(Lags),1));    % Scalar expansion.
   end
   if length(DoF) ~= length(Lags)
      error('econ:lbqtest:DoFLagsLengthMismatch' , ' Sizes of ''DoF'' and ''Lags'' must be the same.');
   end
   if any(DoF > Lags)
      error('econ:lbqtest:DoFTooLarge' , ' No element of ''DoF'' may exceed corresponding element of ''Lags''.');
   end
else
   DoF  =  Lags;
end
